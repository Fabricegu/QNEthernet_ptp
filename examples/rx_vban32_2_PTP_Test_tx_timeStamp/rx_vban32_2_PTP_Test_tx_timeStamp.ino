...
void loop() {
  ...
  if (timerPrint1588 > 500) {
    ...
    if (EthernetIEEE1588.readTimer(ts)) {
      uint32_t seconde = ts.tv_sec;
      uint32_t nano = ts.tv_nsec;
    Serial.printf("[1588] %ld s | %ld ns\n", seconde, nano);

    const int esize = ptpUdp.parsePacket();
    if (esize > 0)
    {
      timespec erecv_ts;
      ptpUdp.timestamp(erecv_ts);
      ptpUdp.parsePacket();
      ptpUdp.timestamp(erecv_ts);
      uint32_t tsec = erecv_ts.tv_sec;
      uint32_t tnsec = erecv_ts.tv_nsec;
      Serial.printf("[timeStamp] %ld s | %ld nano secondes\n", tsec, tnsec);
    }
